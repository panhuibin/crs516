...
public class FileBasedDataServiceTest {
	...
	private File largestFile = new File(DATA_DIR + "SURF_ALL.gz");
...
//This new test checks that multiple threads can use the 
//same FileBasedDataService.	
	@Test
	public void testReadingWithManyThreads() throws InterruptedException, 
															ExecutionException {
		testSubject = createDataService(largestFile);
		ExecutorService threadPool = Executors.newFixedThreadPool(4);
		//Keep the futures so that we know if the assertions were correct.
		List<Future<?>> futureList = new ArrayList<>();
		final long start = System.currentTimeMillis();
		//Submit five Runnables that alternatively read stations 5 and 5000.
		//Since the stations are far apart this will force replenishing of 
		//the cache. A race condition will give incorrect answers. 
		for (int i = 0; i < 5; i++) {
			futureList.add(threadPool.submit(new Runnable() {

				@Override
				public void run() {
					try {//Keep going for 10 seconds
						while (System.currentTimeMillis() - start < 10_000) {
							Station station = testSubject.read(5);
							assertThat(station.getBytesInProfile(),	
																equalTo(384));
							System.out.println("Bytes in profile of station 5:"
									+ station.getBytesInProfile());
							station = testSubject.read(5000);
							assertThat(station.getBytesInProfile(),	
																equalTo(650));
							System.out.println("Bytes in profile of station 5000:"
									+ station.getBytesInProfile());
		...
		//After submitting the Runnables the main thread checks on them.
		for (Future<?> future : futureList) {
			//Even though the Runnable is void, a failed assert would
			//result in an ExecutionException appearing when you call get().
			future.get();
		}
	}

//============================ Old tests after this point =====================
	/**
	...
	private FileBasedDataService createDataService(File file) {
		...
		System.out.println("Loaded "+file.getAbsolutePath()+" and size is "
														+ dataService.size());
		return dataService;
	}
