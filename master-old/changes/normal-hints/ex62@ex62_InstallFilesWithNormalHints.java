...
public class InstallFilesWithNormalHints {
	public static void main(String[] args) {
		...
		File normal_hints_folder = new File(hintsFolderName+fileSeparater+"normal-hints");
		File[] files = normal_hints_folder.listFiles();
			for (File file : files) {
					...
					while((line = reader.readLine() )!=null){
						...
						if(line.startsWith("package")){
							...
							String presentFileAbsolutePath = projectFolderName+fileSeparater
									+"src"+fileSeparater+packageName.replace('.',fileSeparater)
							...
							File fileToStore = new File(hintsFolderName+fileSeparater
									+"reduced-hints"+fileSeparater+file.getName());
			...
			File installFilesWithReducedHintsFile = new File(projectFolderName+fileSeparater+"store"+fileSeparater+"reduced-hints"+fileSeparater+"InstallFilesWithReducedHints.java");
			File targetInstallFile = new File(projectFolderName+fileSeparater+"src"+fileSeparater+"InstallFilesWithReducedHints.java");
			try {
				copyFile(installFilesWithReducedHintsFile, targetInstallFile);
			} catch (IOException e) {
				e.printStackTrace();
			}
			File thisFile = new File(projectFolderName+fileSeparater+"src"+fileSeparater+"InstallFilesWithNormalHints.java");
